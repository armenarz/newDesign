...
/* 					$sql1="	SELECT 
								...
								CONCAT( doctor.LastName,' ', doctor.FirstName,' ', doctor.MidName) AS DocName,
								orders.OrderId,
								CONCAT(pacients.LastName,' ', pacients.FirstName,' ', pacients.MidName ) AS CustName,reagent.AnalysisPrice,
								orders.DoctorDiscount,
								orders.usr, 
								orders.OrderDiscount,
								orders.OrderDate,
							...
							LEFT JOIN reagent ON reagent.ReagentId = orderresult.ReagentId 
							WHERE (orders.OrderDate >= '".$startDate."') AND (orders.OrderDate <= '".$endDate."') AND (orders.DoctorId='".$docid."') AND orders.OrderId='".$orderid."' 
							ORDER BY orders.cena_analizov
							"; */
	...
	$msg.= '
	<table class="table" border="1" id="doctorSelectedData">
		<caption>
            <h2>Врач выбранный</h2>
				...
				for($k = 0; $k < COUNT($array_order); $k++)
				{
					if($current_doctor_id == $array_order[$k][0])
					{
						...
						$msg.= '<tr>
									...
									<td scope="col" class="text-right">'.$current_order_id.'</td>
...
header("Content-Disposition: attachement; filename=doctorsSelectedDetailed.xls");
echo $msg;
?>