...
if($menuId == "doctorsLink" && $reportTypeId == 1)
{
    ...
    else if($doctorId == 0 && $reagentId == 0 && $workplaceId == 0 && $salesId != 0 && $userId != 0)
    {
        if($salesId == 2 || $salesId == 4 || $salesId == 16)
        {
            $filter = "orders.OrderDate>='".$startDate."' AND orders.OrderDate<='".$endDate."' AND doctor.sales_id='".$salesId."' AND (doctor.sales_id='".$salesId."' OR orders.DoctorId='1188' OR orders.DoctorId='1544')";
        }
        else
        {
